    ...
    <section v-if="cartCount > 0">
      <table>
        ...
        <tr v-for="item in cart" :key="item.id">
          <td>
             <v-card flat color="grey lighten-3" max-width="100px" height="100px">
            <img :src="`/products/${item.image}`" :alt="item.name" class="product-img" /></v-card>
            <h3 class="product-name">{{ item.name }}</h3>
           
          </td>
          <td>
            <h4 class="price">&#8377;{{ item.sp}}</h4>
          </td>
          <td>
